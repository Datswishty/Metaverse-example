    ...
    IERC721Consumable
{
    ...
    mapping(uint256 => uint256) private buildingToLand;
    ...
    function initialize() public initializer {
        __ERC721_init("CoolMetaverseName Building", "Building");
        __Pausable_init();
        __Ownable_init();
    ...
    function mintBuilding(address to, string memory _tokenURI) external {
    ...
    function attachBuildingToLand(uint256 buildingTokenId, uint256 landTokenId)
        external
    {
        require(
            msg.sender == ownerOf(buildingTokenId),
            "You are not the owner"
        );
        buildingToLand[buildingTokenId] = landTokenId;
    }

    function deattachBuildingFromLand(uint256 buildingTokenId) external {
        require(
            msg.sender == ownerOf(buildingTokenId),
            "You are not the owner"
        );
        delete buildingToLand[buildingTokenId];
    }
}
